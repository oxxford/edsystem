...
def distribute_tasks(tasks_path, students_path):
    ...
    task_map = {student_id: [] for student_id in
                students_map.keys()}  # [] list of tasks. where task is a array of obj from pages {}
    for task_file in tasks_files:
        ...
        if tasks_type == 'test_set':
            ...
            task_map = set_task(curr_task, students_ids=students_map.keys(),
                                tasks_map=task_map)  # assigns task to each student
            ...
            for task_v in task_variants:
                ...
                task_map = set_task([task_v], students_ids=corresponding_students,
                                    tasks_map=task_map)  # assigns corresponding learners task
    return task_map


def save_tasks(task_map, student_folder, students_folder):
    i = 0
    for student, tasks in task_map.items():
        student_folder = os.path.join(student_folder, student)
        for task in tasks:
            with open(f'{students_folder}/{student}/active_tasks/task{i}.json', 'w+') as file:
                file.write(json.dumps(task))
            i += 1

# students_folder = 'students/grade 1/A'
# task_path = 'subjects/literature_4_2019/lesson1/tasks'
# task_map = distribute_tasks(tasks_path=task_path, students_path=students_folder)
